...
function readConfig(cwd: string): Config {
	try {
		const raw = fs.readFileSync(settingsPath(cwd), "utf-8");
		const parsed = JSON.parse(raw);
		const saved = parsed.workingSpinner;
		if (!saved || typeof saved !== "object") return { ...DEFAULT_CONFIG };
		return {
			...DEFAULT_CONFIG,
			...saved,
			frames: (saved.frames as FramePreset) ?? DEFAULT_CONFIG.frames,
			verbs: (saved.verbs as VerbPreset) ?? DEFAULT_CONFIG.verbs,
		};
	} catch {
...
function writeConfig(cwd: string, config: Config): void {
	...
	try {
		...
		if (exists) {
			...
			const parsed = JSON.parse(raw);
			parsed.workingSpinner = config;
			fs.writeFileSync(sp, JSON.stringify(parsed, null, 2) + "\n");
		} else {
			fs.writeFileSync(sp, JSON.stringify({ workingSpinner: config }, null, 2) + "\n");
		}
	} catch (e) {
		console.error("[working-spinner] Failed to write config:", e);
	}
}
...
export default function (pi: ExtensionAPI) {
	...
	function updateShimmer() {
		...
		if (!accentHex || !shimmerHex) {
			currentCtx.ui.setWorkingMessage(currentCtx.ui.theme.fg("accent", currentVerbText));
			return;
		}
		const colored = colorSweep(currentVerbText, shimmerFrame, accentHex, shimmerHex);
	...
	function showCompletionVerb(ctx: ExtensionContext) {
		...
		const word = ctx.ui.theme.fg("accent", verb);
		ctx.ui.setStatus("spinner", `${check} ${word}`);

		completionTimer = setTimeout(() => {
			completionTimer = null;
			if (!isExtensionActive) return;
			ctx.ui.setStatus("spinner", "ready");
